...
def extract_feature_maps(model_name, device,config):
    ...
    i=0
    root=''
    data_loader = DataLoader(PCNClip(config,i))
    num=0
    for data in tqdm(data_loader):
        taxonomy_id, model_id, pc = data
        partial = pc[0]
        gt = pc[1]
        ...
        with torch.no_grad():
            img_feat,img_feat_block = extractor(partial,gt,model_id[0],taxonomy_id[0])

            clip_feat = (img_feat,img_feat_block)
            # print(img_feat.shape)
            path =  root+'clip_feat/'+taxonomy_id[0]+'/'+model_id[0]+'/'+'0'+str(i)+'.pt'
            foldpath = root+'clip_feat/'+taxonomy_id[0]+'/'+model_id[0]
            if not os.path.exists(foldpath):
                os.makedirs(foldpath)

            torch.save(clip_feat,path)



def main(args):
    config = cfg_from_yaml_file('/home/supermicro/zq/PoinTr/cfgs/extractor.yaml')
    random.seed(0)
    device = "cuda:0"
    ...
    extract_feature_maps(model_name, device,config)

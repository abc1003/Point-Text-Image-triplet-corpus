...
class CrossModalAttention(nn.Module):
    def __init__(self, dim1=128, dim2=64, embed_dim=64):
        super().__init__()
        self.proj1 = nn.Linear(1, embed_dim)
        self.proj2 = nn.Linear(1, embed_dim)
    ...
    def forward(self, x1, x2):
        ...
        x1_proj = self.proj1(x1)  # (B,128,embed_dim)
        x2_proj = self.proj2(x2)  # (B,64,embed_dim)

        q = self.query(x1_proj)  # (B,128,embed_dim)
        k = self.key(x2_proj)  # (B,64,embed_dim)
        v = self.value(x2_proj)  # (B,64,embed_dim)

        attn_scores = torch.bmm(q, k.transpose(1, 2))  # (B,128,64)
        attn_scores = attn_scores / (embed_dim ** 0.5)

        attn_weights = F.softmax(attn_scores, dim=-1)

        context = torch.bmm(attn_weights, v)  # (B,128,embed_dim)

        fused = self.out(context)  # (B,128,1)
...
class PCTransformer(nn.Module):
    ...
    def forward(self, xyz, clip_f):
        bs = xyz.size(0)
        coor, f = self.grouper(xyz, self.center_num)  # b n c

        clip_f = self.conv1(clip_f)
        clip_f = clip_f.repeat(1, 1, 256)

        new_f = torch.cat((f.permute(0, 2, 1), clip_f), dim=1)
        new_f = self.conv2(new_f)
        new_f = new_f.permute(0, 2, 1)

        pe = self.pos_embed(coor)
        x = self.input_proj(new_f)
...
class AdaPoinTr(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv1d(128 +32+ 64, 64, 1),
            nn.BatchNorm1d(64),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv1d(64, 32, 1)
        )
        self.conv3 = nn.Sequential(
            nn.Conv1d(1024 + 64, 1024, 1),
            nn.BatchNorm1d(1024),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv1d(1024, 1024, 1)
        )
        self.conv128 = nn.Sequential(
            nn.Conv1d(3072, 256, 1),
            nn.BatchNorm1d(256),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv1d(256, 128, 1)
        )
        self.conv16 = nn.Sequential(
            nn.Conv1d(3072, 128, 1),
            nn.BatchNorm1d(128),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv1d(128, 16, 1)
        )
        self.ca = CrossModalAttention()
    ...
    def forward(self, xyz, clip_feat):

        text_f = clip_feat[0].reshape(-1, 512, 1)
        glbalimg, localimg = clip_feat[1]
        glbalimg = glbalimg.float()
        localimg = localimg.float()
        glbalimg_feat = self.conv128(glbalimg.reshape(-1, 512 * 6, 1))
        localimg_feat = self.conv16(localimg.reshape(-1, 512 * 6, 4))
        localimg_feat = localimg_feat.reshape(-1, 64, 1)
        caimg = self.ca(glbalimg_feat,localimg_feat)
        caimg1 = self.conv4(caimg)
        img_feat = torch.cat((glbalimg_feat, localimg_feat,caimg1), dim=1)
        text_f = self.conv1(text_f)
        img_f_c = self.conv2(img_feat)
        clip_f = torch.cat((text_f, img_f_c), dim=1)

        q, coarse_point_cloud, denoise_length = self.base_model(xyz, clip_f)  # B M C and B M 3

        B, M, C = q.shape

        global_feature = self.increase_dim(q.transpose(1, 2)).transpose(1, 2)  # B M 1024
        global_feature = torch.max(global_feature, dim=1)[0]  # B 1024
        global_f = torch.cat((global_feature.reshape(B, 1024, 1), clip_f), dim=1)
        global_f = self.conv3(global_f)
        global_feature1 = global_f.reshape(B, 1024)

        rebuild_feature = torch.cat([
            global_feature1.unsqueeze(-2).expand(-1, M, -1),
            q,

...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        # for idx, (taxonomy_ids, model_ids, data) in enumerate t(train_dataloader):
        for idx, (taxonomy_ids, model_ids, data) in enumerate(tqdm(train_dataloader)):
            data_time.update(time.time() - batch_start_time)
            npoints = config.dataset.train._base_.N_POINTS
            dataset_name = config.dataset.train._base_.NAME
            if dataset_name == 'PCN' or dataset_name == 'PCNv2' or dataset_name == 'Completion3D' or dataset_name == 'Projected_ShapeNet':
                partial = data[0].cuda()
                gt = data[1].cuda()
                global_feat,local_feat = data[2]
               
                clip_feat = (global_feat.cuda(),local_feat.cuda())
            ...
            clip_feat1 = (text_feat,clip_feat)
            
           
            ret = base_model(partial,clip_feat1)
...
def test(base_model, test_dataloader, ChamferDisL1, ChamferDisL2, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if dataset_name == 'PCN' or dataset_name == 'PCNv2' or dataset_name == 'Projected_ShapeNet':
                partial = data[0].cuda()
                gt = data[1].cuda()
                global_feat,local_feat = data[2]
                bs = 1

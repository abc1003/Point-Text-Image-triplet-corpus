...
classid = ['02691156','02933112', '02958343', '03001627', '03636649', '04256520', '04379243', '04530566']

def mv_proj(pc):
    pc_views = projection.PCViews()
...
def get_classname():
    path = 'shapenet_synset_dict.json'
    with open(path) as f:
        data = json.load(f)
        classname = list(data.values())
        classid = list(data.keys())

    return classname, classid,data
...
class Manager:
    ...
    def __init__(self, model, cfg):
        ...
        self.visual_encoder = self.clip_model.visual.cuda()
    ...
    def train(self, model, train_data_loader, val_data_loader, cfg):
        ...
        for epoch_idx in range(init_epoch + 1, cfg.TRAIN.N_EPOCHS + 1):
            ...
            for batch_idx, (taxonomy_ids, model_ids, data) in enumerate(tqdm(train_data_loader)):
                ...
                bs = partial.shape[0]
                p_images = mv_proj(partial)  # (6*b,3,224,224)
                p_images = p_images.to(torch.float16)
                # visual_encoder = clip_model.visual
                image_class_feat,image_feat  = self.visual_encoder(p_images)  # (6*b,196，512)

                image_feat = image_feat.reshape(-1, 196, 3072)  # (b,196,3072)--(B,N,F)
                text_feat_bank = torch.load('tools/PCN_class_textfeat.pt').cuda()
                text_feat = torch.empty(bs, 512).cuda()
                for idx1, i in enumerate(taxonomy_ids):
                    textfeat = text_feat_bank[classid.index(i)]
                    text_feat[idx1] = textfeat

                text_feat = text_feat / text_feat.norm(dim=-1, keepdim=True)
                image_class_feat = image_class_feat / image_class_feat.norm(dim=-1, keepdim=True)  # (b,196,512*6)

                text_feat = text_feat.float()
                image_feat = image_feat.float()
                image_class_feat = image_class_feat.float()

                # print(image_feat.shape)

                clip_feat = (text_feat,image_class_feat, image_feat)
                # print(clip_feat[1].shape)
                pcds_pred = model(partial,clip_feat)
    ...
    def validate(self, cfg, model=None, val_data_loader=None, outdir=None):
        ...
        for model_idx, (taxonomy_id, model_id, data) in enumerate(val_data_loader):
            ...
            with torch.no_grad():
                ...
                # forward
                pcds_pred = model(partial.contiguous(),clip_feat)
...
class Manager_MVP:
    ...
    def train(self, model, train_data_loader, val_data_loader, cfg):
        ...
        for epoch_idx in range(init_epoch + 1, cfg.TRAIN.N_EPOCHS + 1):
            ...
            n_batches = len(train_data_loader)
            learning_rate = self.optimizer.param_groups[0]['lr']
            for batch_idx, (label, partial, gt) in enumerate(tqdm(train_data_loader)):
                #for k, v in data.items():
                #    data[k] = utils.helpers.var_or_cuda(v)

...
from models.utils import PointNet_SA_Module_KNN, MLP_Res, MLP_CONV, fps_subsample, query_knn, grouping_operation, \
...
class CRAPCN(nn.Module):
    ...
    def forward(self, xyz, clip_feat):
        ...
        global_f = torch.cat((global_f, text_f, img_f_c), dim=1)
...
class CRAPCN_mvp(nn.Module):
    ...
    def forward(self, xyz, clip_feat):
        ...
        localimg_feat = self.conv16(localimg.reshape(-1, 512 * 6, 4))
        localimg_feat = localimg_feat.reshape(-1, 64, 1)
        img_feat = torch.cat((glbalimg_feat, localimg_feat), dim=1)
